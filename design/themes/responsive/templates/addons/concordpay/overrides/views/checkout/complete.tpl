{* This template is overridden by the ConcordPay plugin *}
<div class="ty-checkout-complete__order-success">
    <p> {if $payment_status === 'approve'}
            {__("concordpay.payment_approve_text")}
        {elseif $payment_status === 'decline'}
            {__("concordpay.payment_decline_text")}
        {elseif $payment_status === 'cancel'}
            {__("concordpay.payment_cancel_text")}
        {else }
